...
* `allowed_repositories` is a list of strings with full names (`"ORG/REPO"`) of the GitHub repositories that are allowed to trigger Hydra builds.
* `check_run_name` is the name of the check that will be created on each commit.
* `hydra_url` is the base URL for Hydra instance where jobsets will be created.
* `hydra_user` and `hydra_password_env` are user name and the name of the environment variable containing the password that will be used to authenticate in Hydra.
* `hydra_project` is the name of the project in Hydra where jobsets will be created and watched.
...
```json
{
    ...
    "allowed_repositories": ["NixOS/nixpkgs"],
    "check_run_name": "Hydra check",
    "hydra_url": "https://hydra.example.org",
    "hydra_user": "admin",
    "hydra_password_env": "HYDRA_PASSWORD",
    "hydra_project": "pr-tests",
